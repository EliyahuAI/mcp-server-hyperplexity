...
async function handleShareTable(cardId, button, data) {
    ...
    if (action === 'copy') {
        // Copy existing link
        const shareUrl = `${window.location.origin}${window.location.pathname}?demo=${existingDemoName}`;
        try {
    ...
    try {
        ...
        // Build the share URL
        const shareUrl = `${window.location.origin}${window.location.pathname}?demo=${result.table_name}`;
