...
if (data.mode === 1) {
    ...
    createButtonRow(`${cardId}-buttons`, [
        {
            ...
            callback: async (e) => {
                ...
                if (isSimpleConfirmation && confirmation) {
                    // Use pre-generated confirmation - skip showing duplicate messages
                    ...
                    // Show hardcoded AI confirmation message (consistent with backend)
                    const confirmationMessage = "On it! Formalizing the table structure and developing a validation approach. I will then validate the first 3 rows (3-4 minutes) - hang tight.";
                    await addChatMessage(cardId, 'ai', confirmationMessage);

                    // Hide input and show thinking
                    if (inputContainer) inputContainer.style.display = 'none';
                    showThinkingInCard(cardId, 'Starting configuration generation...');

                    // Mark that we already showed confirmation (prevents duplicate from WebSocket)
                    window[`${cardId}_confirmation_shown`] = true;
    ...
    // Only show message if we didn't already show it from stored confirmation
    if (!window[`${cardId}_confirmation_shown`]) {
        await addChatMessage(cardId, 'ai', data.ai_message);
    }
    // Clear the flag
    window[`${cardId}_confirmation_shown`] = false;
...
window.startUploadInterviewFromConfig = async function() {
    ...
    // Find the current configuration card
    const configCard = document.querySelector('[data-card-id]');
    const cardId = configCard ? configCard.getAttribute('data-card-id') : null;

    // Collapse the configuration card to green success box
    if (cardId) {
        showFinalCardState(cardId, 'Creating configuration with AI...', 'success');
    }
    ...
    // Create the upload interview card after brief delay
    // Pass null as previousCardId since we already collapsed the card above
    const uploadData = {
        conversation_id: conversationId,
