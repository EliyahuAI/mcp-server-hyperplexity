...
try {
    ...
    if (previewData.enhanced_download_url) {
        ...
        // The S3 Content-Disposition header forces download, iframe stays blank
        const iframe = document.createElement('iframe');
        iframe.style.display = 'none';
        iframe.src = previewData.enhanced_download_url;
        document.body.appendChild(iframe);

        // Clean up iframe after download starts (wait a bit for download to trigger)
        setTimeout(() => {
