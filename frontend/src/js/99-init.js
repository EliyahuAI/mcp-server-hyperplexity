 ...
 * ======================================== */
        ...
        // On reload: beforeunload fires, then load happens within ~100ms
        // On back/forward: beforeunload fires, user navigates away, comes back later (>500ms)
        // On fresh load: no navigation timestamp exists
        function isPageReload() {
            ...
            // If less than 500ms since beforeunload, this is a reload
            // Back/forward navigation takes longer (user was on another page)
            const isReload = timeSinceNav < 500;
...
if (hasState) {
    try {
        ...
        if (cardCount >= 2) {
            ...
            // isPageReload() uses timing to distinguish:
            // - Reload: beforeunload → load happens within ~100ms
            // - Back/forward: user navigates away, comes back later (>500ms)
            if (isPageReload()) {
                console.log('[STATE] Page reload detected, clearing saved state to start fresh');
                sessionStorage.removeItem(stateKey);
                return false;
            }
...
if (!document.hidden) {
    ...
    setTimeout(() => {
        ...
        if (window.initialRestoreDecisionMade || hasValuableData() || window.isResetting) {
