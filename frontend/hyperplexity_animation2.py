...
SCALE_FACTOR = 4  # Smaller scale for compact GIF
BASE_SIZE = 140  # Smaller base to reduce whitespace
CANVAS_SIZE = int(BASE_SIZE * SCALE_FACTOR*0.92)  # Actual render size = 280px
...
GREEN = (45, 255, 69, 128)  # #2DFF45 with 80% opacity
BLACK = (0, 0, 0, 255)
WHITE = (255, 255, 255)
...
def create_frame(frame_num):
    ...
    if progress <= 0.1:  # Initial state - static square
        ...
        visual_half_height = half_height * math.cos(viewing_angle_rad)*1.25
...
def generate_gif():
    ...
    frames[0].save(
        ...
        duration=50,
        loop=1  # Set to 1 so the GIF does not loop infinitely (plays once)
    )
