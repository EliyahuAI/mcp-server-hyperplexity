        ...
        with xlsxwriter.Workbook(excel_buffer, {'strings_to_urls': False, 'nan_inf_to_errors': True}) as workbook:
                    ...
                    for col_idx, col_name in enumerate(headers):
                        ...
                        if row_validation_data and isinstance(row_validation_data, dict):
                            ...
                            if should_apply_coloring(col_name):
                                ...
                                # Special rule: blank-like values with LOW confidence should be uncolored (forced to NULL)
                                blank_like_values = ('', '-', '—', '–', 'n/a', 'na', 'null', 'none')
                                is_blank_like = (original_value is None or
                                                str(original_value).strip().lower() in blank_like_values)
                                if is_blank_like and str(original_confidence).strip().upper() == 'LOW':
            ...
            if INCLUDE_QC_COLUMNS:
                detail_headers.extend(["Column", "Original Value", "Updated Value", "QC Value",
                                ...
                                "QC Applied", "QC Reasoning", "Update Importance", "Key Citation",
                                "Final Value", "Citations",
                ...
                try:
                    ...
                    for field_name, field_data in row_validation_data.items():
                        if isinstance(field_data, dict) and 'confidence' in field_data:
                            ...
                            # Key Citation column - QC's key citation that references validation citations as [V1], [V2]
                            details_sheet.write(detail_row, col_idx, safe_for_excel(key_citation))  # Key Citation
                            col_idx += 1
                            ...
                            # Citations column - validation citations numbered [1], [2], etc.
                            # Key Citation column above uses [V1], [V2] to reference these
                            try:
                                ...
                                logger.warning(f"Citations formatting failed: {e}")
                                citations_text = ''
