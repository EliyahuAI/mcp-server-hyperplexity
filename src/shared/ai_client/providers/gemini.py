...
def get_gemini_semaphore(model: str) -> asyncio.Semaphore:
    ...
    with _gemini_semaphore_lock:
        if key not in _gemini_semaphores:
            ...
            # Use insertion order (Python 3.7+ dict) to find the true oldest loop, not
            # min() which compares memory addresses and may wrongly select the current loop.
            loop_ids = set(k[0] for k in _gemini_semaphores.keys())
            if len(loop_ids) > 2:
                oldest_loop = next((k[0] for k in _gemini_semaphores.keys() if k[0] != loop_id), None)
                if oldest_loop is not None:
