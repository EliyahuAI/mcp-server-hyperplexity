    ...
    with _gemini_semaphore_lock:
        if key not in _gemini_semaphores:
            ...
            if len(loop_ids) > 2:
                ...
                # Don't delete the current loop's semaphores (prevents KeyError on line 74)
                if oldest_loop != loop_id:
                    ...
                    logger.debug(f"[GEMINI_SEMAPHORE] Skipping cleanup of current loop {loop_id} (it's the oldest)")
