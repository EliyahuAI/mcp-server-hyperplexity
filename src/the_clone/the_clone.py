...
from the_clone.config import get_optimization_guidance
from the_clone.config_loader import get_context_config, should_skip_evaluation
...
class TheClone2Refined:
    ...
    async def query(
        ...
        # Load configuration (will be overridden by initial decision)
        config = get_context_config(search_context, variant=config_variant)
        triage_model = config.get('triage_model', 'claude-haiku-4-5')
        extraction_model = config.get('extraction_model', 'claude-haiku-4-5')
        initial_decision_model = config.get('initial_decision_model', 'claude-sonnet-4-5')
        use_soft_schema = config.get('use_soft_schema', False)

        logger.info(f"[CLONE 2 REFINED] Initial context (will be overridden): {search_context}")
        logger.info(f"[CLONE 2 REFINED] Using models: initial={initial_decision_model}, triage={triage_model}, extraction={extraction_model}, soft_schema={use_soft_schema}")
            ...
            try:
                with open(os.path.join(debug_dir, '00_config.json'), 'w', encoding='utf-8') as f:
                    json.dump({
                        ...
                        'models': {
                            'initial_decision': initial_decision_model,
                            'triage': triage_model,
                            'extraction': extraction_model,
                            'use_soft_schema': use_soft_schema
                        },
        ...
        initial_result = await self.initial_decision.make_decision(
            query=prompt,
            model=config.get('initial_decision_model', 'claude-sonnet-4-5'),
        ...
        # Use determined context and model tier from initial decision
        determined_context = initial_result.get('search_context', search_context)
        synthesis_tier = initial_result.get('synthesis_model_tier', 'strong')

        # Reload config with determined context to get the right model tiers
        config = get_context_config(determined_context, variant=config_variant)

        # Get model tiers from config (already set correctly by get_context_config)
        model_tiers = config.get('model_tiers', {})
        determined_synthesis_model = model_tiers.get(synthesis_tier, 'claude-sonnet-4-5')
        max_iterations = config['max_iterations']
        sources_per_search_min, sources_per_search_max = config.get('sources_per_search_range', [1, 3])
        max_results = config.get('max_results_per_search', 10)

        logger.info(f"[CLONE 2 REFINED] Determined context: {determined_context}, Model tier: {synthesis_tier} ({determined_synthesis_model})")
