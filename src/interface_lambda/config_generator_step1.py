...
class TableAnalyzer:
    ...
    def analyze_table(self, file_path, sheet_name=None):
        ...
        try:
            if file_path.endswith('.csv'):
                ...
                print(f"Loaded Excel sheet '{target_sheet}': {df.shape[0]} rows, {df.shape[1]} columns")
            else:
    ...
    def _analyze_structure(self, df, file_path):
        ...
        for col in df.columns:
            ...
            print(f"   {col}: {col_info['data_type']}, {col_info['unique_count']} unique, {col_info['null_count']} nulls")
