...
```
config_lambda/                         # Dedicated config lambda folder
...
├── perplexity_schema.py               # Perplexity API schema definitions
├── column_config_schema.json          # JSON schema for config validation
├── requirements.txt                   # Python dependencies for deployment
├── CONFIG_LAMBDA_README.md            # This documentation file
└── prompts/
...
### Config Lambda Folder Structure
The `config_lambda/` folder contains all components needed for deployment:

```
config_lambda/
├── config_lambda_function.py          # Main lambda handler
├── ai_api_client.py                   # Shared AI API client
├── perplexity_schema.py               # API schemas
├── column_config_schema.json          # Config validation schema
├── requirements.txt                   # Python dependencies
├── CONFIG_LAMBDA_README.md            # This documentation
└── prompts/
    ├── create_new_config_prompt.md     # New config creation prompt
    ├── refine_existing_config_prompt.md # Config refinement prompt
    └── generate_column_config_prompt.md # Original prompt (updated)

Note: shared_table_parser.py is copied from src/ during deployment to avoid duplication
```

### Deployment Process
The deployment script `deployment/create_interface_package.py` automatically:
1. **Copies config lambda files** from `config_lambda/` folder to build directory
2. **Copies shared resources** from `src/` (like `shared_table_parser.py`) to avoid duplication
3. **Installs dependencies** from `config_lambda/requirements.txt`
4. **Creates ZIP package** for AWS Lambda deployment
5. **Deploys to AWS** with proper configuration

### Usage
```bash
# Deploy all components including config lambda
python deployment/create_interface_package.py --deploy

# Test the deployed config lambda
python test_config_lambda.py --test-all
```
