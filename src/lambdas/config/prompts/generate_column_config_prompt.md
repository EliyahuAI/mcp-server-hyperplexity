...
- **Field**: `sheet` (string, optional, root-level)
- **Purpose**: Specifies the sheet name to use if the table is an Excel file.
- **Default Behavior**:
  - Use the sheet named `'Updated'` if it exists (this is typically the results of a past validation).
  - If `'Updated'` is not present, use the first sheet in the file **except** if it is named `'Details'` (in which case, use the next available sheet).
  - Avoid using the `'Details'` sheet for validation.
- **When to Specify**: Only include this field if you need to override the default behavior or the correct sheet is ambiguous.
...
```json
{
  ...
  "default_model": "sonar-pro",
  "default_search_context_size": "low",
  "sheet": "Updated", // Optional: specify the sheet to use if Excel file
  "validation_targets": [
    {
      "column": "Product Name",
    ...
    },
    {
      ...
      "search_group": 4,
      "preferred_model": "claude-sonnet-4-0",
      "search_context_size": "high"
    }
  ]
} 