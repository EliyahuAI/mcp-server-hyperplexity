...
def copy_demo_to_session(demo_name: str, email: str, old_session_id: str) -> Dict:
    ...
    try:
        ...
        # Copy config file using standard naming pattern
        config_source_key = demo_metadata['config_file']['s3_key']
        config_dest_key = f"{session_prefix}config.json"

        s3_client.copy_object(
            Bucket=bucket_name,
            CopySource={'Bucket': bucket_name, 'Key': config_source_key},
            Key=config_dest_key
        )

        # Read config content to return for frontend
        ...
        session_info = {
            ...
            "versions": {
                "1": {
                    "version": 1,
                    "config": {
                        "config_id": f"demo_{demo_name}_v1",
