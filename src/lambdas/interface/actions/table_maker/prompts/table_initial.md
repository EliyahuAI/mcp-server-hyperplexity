...
6. **Populate confirmation_response**: Pre-generate the response that will be shown if user approves
   ...
   - `confirmation_response.ai_message`: Brief confirmation like "Great! Starting table generation now. Preview typically takes 5–15 minutes depending on rows, search difficulty, and columns..."
...
- DO NOT mention time estimates in ai_message (save that for confirmation_response.ai_message)
