...
class TableConversationHandler:
    ...
    def _extract_api_metadata(self, api_response: Dict[str, Any]) -> Dict[str, Any]:
        """
        Extract cost and metadata from AI API response for runs table tracking.

        Args:
            api_response: Full API response from call_structured_api

        Returns:
            Dictionary with eliyahu_cost, models, token_usage, and timing data
        """
        metadata = {
            'eliyahu_cost': 0.0,
            'models': '',
        ...
        try:
            ...
            if enhanced_metrics:
                ...
                metadata['eliyahu_cost'] = actual_cost.get('total_cost', 0.0)
                ...
                logger.debug(
                    f"Extracted API metadata - Cost: ${metadata['eliyahu_cost']:.6f}, "
                    f"Model: {metadata['models']}, Tokens: {metadata['token_usage'].get('total_tokens', 0)}"
                )
            else:
                # Fallback to basic token usage if enhanced_metrics not available
                token_usage = api_response.get('token_usage', {})
                if token_usage:
                    metadata['token_usage'] = token_usage
                    metadata['models'] = api_response.get('model_used', 'unknown')
                    logger.warning("Enhanced metrics not available, using basic token usage")

        except Exception as e:
            logger.error(f"Error extracting API metadata: {e}")
            # Return empty metadata on error rather than failing
