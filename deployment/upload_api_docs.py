...
Targets:
  ...
  docs/API_GUIDE.md   → s3://{bucket}/website_downloads/API_GUIDE.md
...
Usage:
    ...
    python upload_api_docs.py --upload --bucket my-bucket \
        --examples ../mcp/examples --api-guide ../docs/API_GUIDE.md
"""
...
DEFAULTS = {
    ...
    "api_guide":    REPO_ROOT / "docs" / "API_GUIDE.md",
}
...
def build_manifest(examples_dir: Path, api_guide: Path) -> List[Tuple[Path, str]]:
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        metavar="FILE",
        help=f"Path to API_GUIDE.md (default: {DEFAULTS['api_guide']})",
    )
    parser.add_argument(
        "--bucket",
        default=BUCKET,
    ...
    manifest = build_manifest(examples_dir, api_guide)
