...
def handle_main_processing(event, context):
    ...
    try:
        ...
        try:
            ...
            logger.error(f"[MEMORY_SETUP] Failed to set session context in background: {e}", exc_info=True)
        ...
        if has_results:
            ...
            try:
                ...
                # Use effective_cost (after discount) for billing and email display
                charged_amount = effective_cost
                logger.debug(f"[BILLING_INIT] charged_amount set to effective_cost: ${charged_amount:.2f} (original: ${charged_cost:.2f}, discount: ${discount:.2f})")
